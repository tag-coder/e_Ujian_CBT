...
import { 
  ...
  Users, TrendingUp, Settings, ChevronRight, Plus, Info,
  BookMarked, Key, ArrowRight, Clock, Flag, Shield
} from 'lucide-react';
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-64 bg-slate-50 border-r border-slate-200 hidden md:flex flex-col z-40">
      ...
      <div className="p-4 border-t border-slate-200 space-y-1">
        <button className="w-full flex items-center gap-3 px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg text-sm font-medium transition-all font-sans">
          <Settings className="w-5 h-5" />
          Settings
        </button>
        <button 
          onClick={() => setView('login')}
          className="w-full flex items-center gap-3 px-4 py-2 text-rose-600 hover:bg-rose-50 rounded-lg text-sm font-medium transition-all font-sans"
        >
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
      ...
      >
        <div className="text-center mb-12">
          ...
          <p className="text-slate-500 text-lg">Integrated Examination Information System</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow group">
            ...
            <div className="space-y-4">
              <input type="text" placeholder="Email / NIP" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary outline-none" />
              <input type="password" placeholder="Password" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary outline-none" />
              <button 
                onClick={() => setView('admin-results')}
                className="w-full py-4 bg-primary text-white font-bold rounded-xl hover:bg-blue-700 transition-all flex items-center justify-center gap-2"
              >
          ...
          <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">Access examination hall</p>
              </div>
            </div>
            <div className="space-y-4">
              <input type="text" placeholder="NIS (Student ID)" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-slate-400 outline-none" />
              <input type="password" placeholder="Password" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-slate-400 outline-none" />
              <button 
          ...
          <div className="lg:col-span-4 bg-white rounded-3xl border border-slate-200 p-8 flex flex-col justify-between shadow-sm">
            <div className="flex items-center gap-4">
              <div className="w-20 h-20 rounded-2xl overflow-hidden shadow-inner">
                <img src="https://lh3.googleusercontent.com/aida-public/AB6AXuCFx_EnfmIUB1OiO5AHEQvOTUXadYk2ku17ayYt0IcSTlf0pauMWU51-qNQpguBcNIn8wsCAKZI1uz42q69K9-FxjhkTreF8s7cn_qBXvvedeDvCgCTtiCzI4SggpIH60GOK8pqd_smnxJxepgZGRa9fUfRxYkQ2jWmsmc5GDDxgjn2xzINRBDbdFO40o1Qry7XSVD8di10lDmJA8R8piSk9sZJjP1rWsT0eLoA2UnrqH5uIYjaS_h2mj8XewiACukDGCMLwIvB2Z4C" className="w-full h-full object-cover" alt="Student" />
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="p-8 space-y-4">
                <div className="p-5 rounded-2xl bg-blue-50 border border-blue-100 flex gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-blue-700">The schedule for 2024 Final Exams is now live.</p>
                  </div>
                </div>
              </div>
            </section>
          ...
          <div className="lg:col-span-4 space-y-8">
            <section className="bg-white rounded-3xl border border-slate-200 p-8">
              ...
              <button 
                onClick={() => setView('exam-session')}
                className="w-full py-4 bg-primary text-white font-bold rounded-2xl hover:bg-blue-700 shadow-lg shadow-blue-500/20 active:scale-95 transition-all"
              >
                Start Session
              </button>
            </section>
        ...
        <div className="flex flex-wrap items-end justify-between gap-6 bg-white p-8 rounded-3xl border border-slate-200">
          <div className="flex flex-wrap items-center gap-6">
            <div className="space-y-2">
              ...
              <select className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium outline-none font-sans"><option>All Classes</option></select>
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Session</label>
              <select className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium outline-none font-sans"><option>Mid-Term Semester 2</option></select>
            </div>
          </div>
          <div className="flex gap-4">
            <button className="px-6 py-2.5 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-all font-sans">Print</button>
            <button className="px-6 py-2.5 bg-primary text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-500/20 transition-all font-sans">Export</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 bg-primary rounded-3xl p-10 text-white relative overflow-hidden shadow-2xl">
            <div className="relative z-10 flex flex-col h-full">
              ...
              <p className="text-blue-100 text-sm mb-10 flex-1 opacity-90 leading-relaxed">Generate personalized achievement certificates for students.</p>
              <button className="w-full py-4 bg-white text-primary font-black rounded-2xl hover:bg-slate-50 transition-all uppercase tracking-widest text-xs font-sans">Process Reports</button>
            </div>
          </div>
        </div>
      </main>
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-[1440px] mx-auto p-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-8">
            ...
            <div className="space-y-10">
              ...
              <div className="grid grid-cols-1 gap-4 font-sans">
                {['A. 4x² + 12x + 8', 'B. 4x² + 12x + 9', 'C. 4x² + 6x + 8', 'D. 2x² + 3x - 1'].map((opt, i) => (
                  <button key={i} onClick={() => setSelected(i)} className={`p-6 border-2 rounded-3xl transition-all text-left ${selected === i ? 'border-primary bg-blue-50 text-primary' : 'border-slate-50 hover:bg-slate-50 text-slate-600'}`}>
          ...
          <div className="bg-white rounded-[24px] border border-slate-200 p-6 flex flex-wrap gap-4 items-center justify-between shadow-sm font-sans">
            <div className="flex gap-4">
          ...
          <div className="bg-white rounded-[32px] border border-slate-200 p-8 shadow-sm">
            ...
            <div className="grid grid-cols-5 gap-3">
              {Array.from({length: 25}, (_, i) => i + 1).map(n => (
                <div key={n} className={`aspect-square rounded-xl flex items-center justify-center font-bold text-sm cursor-pointer transition-all ${n === 24 ? 'border-2 border-primary text-primary shadow-lg scale-110' : n < 24 ? 'bg-primary text-white' : 'bg-slate-50 text-slate-300'}`}>{n}</div>
              ))}
            </div>
          </div>
        </div>
